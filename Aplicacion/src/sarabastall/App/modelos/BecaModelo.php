<?php
    class BecaModelo{
        ...
        public function anadirBeca($datos){
            ...
            $this->db->query("INSERT INTO beca(importe, Obs, fechaInicio, fechaFin, idTipo, idCentro, idPersona, fechaAbonoMadrina, idAlumno, fechaAlumno)
                                    VALUES (:importe, :Obs, :fechaInicio, :fechaFin, :idTipo, :idCentro, :idPersona, :fechaAbonoMadrina, :idAlumno, :fechaAlumno)");
            print_r($datos);
        ...
        public function editarBeca($becaModificado, $idBeca){
            ...
            $this->db->query("UPDATE beca 
                                SET importe=:importe, Obs=:Obs, fechaInicio=:fechaInicio, fechaFin=:fechaFin, idTipo=:idTipo, idCentro=:idCentro, idPersona=:idPersona, fechaAbonoMadrina=:fechaAbonoMadrina,
                                    idAlumno=:idAlumno, fechaAlumno=:fechaAlumno
                                WHERE idBeca=:idBeca");
        ...
        public function obtenerMovimientos($idBeca) {
    ...
    public function anadirMovimiento($movimiento){
            ...
            $this->db->query("INSERT INTO movimiento(procedencia, accion, fecha, cantidad, idTipo, idBeca)
                VALUES('beca', :accion, NOW(), :cantidad, 1, :idBeca)");

            $this->db->bind(':accion',$movimiento['accion']);
            $this->db->bind(':cantidad', $movimiento['cantidad']);
            $this->db->bind(':idBeca',$movimiento['idBeca']);
